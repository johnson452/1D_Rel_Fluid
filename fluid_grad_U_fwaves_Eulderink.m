...
function [Ux,Uy,Uz,Vx,Vy,Vz,N,grid] = fluid_grad_U_fwaves_Eulderink(Ux,Uy,Uz,N,grid)
...
vhat = v_hat_via_eulderink_cold_limit(N,Ux,Uy,Uz,L,grid.c);

% diagnostic for v_hat
diag_vhat(vhat,Ux,Uy,Uz,grid);

%Calulcate fluxes:
FG_L = Flux_Godunov(U_prim,vhat,grid);
FG_R =  FG_L(:,R);

%Calculate the corrector Fluxes
[FCorr_L,grid] = Flux_Corr(U_prim,vhat,grid);
FCorr_R = FCorr_L(:,R);
...
function diag_vhat(vhat,Ux,Uy,Uz,grid)

if mod(grid.iter,1000) == 0
    figure2 = figure();
    gamma = sqrt(1 + (Ux.^2 + Uy.^2 + Uz.^2)/(grid.c^2));
    v = Ux./gamma;
    plot(grid.x2(grid.L), vhat, '.black')
    hold on
    plot(grid.x1, v, '*red')
    title('v-hat diagnostic')
    xlabel('x-coord.')
    ylabel('v.')
    legend('v-hat', 'Vx')
    pause(5)
...
function [vhat] = v_hat_via_eulderink_cold_limit(N,Ux,Uy,Uz,L,c_light)
% Nonlinear solve:
%Constants
c = c_light;
rho_max = max(N(L),N);
...
%Avg state:
%k = sqrt(rhol_prim) + sqrt(rhor_prim);
w0 = sqrt(rhol_prim).*gammal + sqrt(rhor_prim).*gammar;
w1 = sqrt(rhol_prim).*gammal.*vlx + sqrt(rhor_prim).*gammar.*vrx;
%w2 = sqrt(rhol_prim).*gammal.*vly + sqrt(rhor_prim).*gammar.*vry;
%w3 = sqrt(rhol_prim).*gammal.*vlz + sqrt(rhor_prim).*gammar.*vrz;
...
function [Fl] = Flux_Godunov(Q,vhat,grid)
...
function [Fl,grid] = Flux_Corr(Q,vhat,grid)
